...
// Per-virtual-page metadata for fragmentation reduction
// kind: 0 = unused, 1 = large (page-based) allocation, 2 = small-allocation pool
typedef struct {
    uint8_t  kind;
    uint32_t active_suballocs;   // number of live small allocations in this page
    uint32_t next_free_off;      // next free offset inside the page (simple bump allocator)
} vpage_meta_t;

static vpage_meta_t *vpage_meta = NULL;
