...
// Per-page metadata to reduce fragmentation
// kind: 0 = unused, 1 = large allocation (full page), 2 = small allocation pool
typedef struct {
    uint8_t  kind;
    uint32_t active_suballocs;   // how many small allocations are still live in this page
    uint32_t next_free_off;      // next available offset in the page (simple bump allocator)
} vpage_meta_t;

extern vpage_meta_t *vpage_meta;
