...
static void tlb64_invalidate_vpn(uint64_t vpn) {
    uint64_t idx = vpn % TLB64_ENTRIES;
    if (tlb64[idx].valid && tlb64[idx].vpn == vpn) {
        tlb64[idx].valid = false;
    }
}
...
static void unmap_pages(vaddr64_t va, size_t pages) {
    for (size_t k = 0; k < pages; k++) {
        ...
        uint64_t vpn = cur >> OFF64_BITS;
        tlb64_invalidate_vpn(vpn);
    }
}
...
void vm64_init(void) {
    ...
    if (phys_mem64 == MAP_FAILED) {
        perror("vm64_init: mmap for MEMSIZE64 failed");
        phys_mem64 = NULL;
        pthread_mutex_unlock(&vm64_mutex);
        return;
    }
...
vaddr64_t vm64_malloc(size_t bytes) {
    ...
    for (uint64_t i = 0; i < pages; i++) {
        ...
        void *host = pfn_to_host(pfn);
        memset(host, 0, PGSIZE64);
    }
